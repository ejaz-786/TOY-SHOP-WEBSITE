    ...
    console.log(e);
      ...
      searchdata.cartArr.forEach((obj) => {
        ...
        if(obj.id === e){
          console.log('agaya');
            obj.Quan++;
            obj.total = (Toy[indx].price - (Toy[indx].price * Toy[indx].discount) / 100) *obj.Quan;
            flag = false;
        }
        Toy[e].isWished = false;
        searchdata.setWishLIst(searchdata.wishList-1);
        searchdata.setTotal(searchdata.total + (Toy[indx].price - (Toy[indx].price * Toy[indx].discount) / 100)
      );
  ...
  return (
    <>
      <div className="nav">
        <div className="navbar">
          ...
          <div className="head">
            <Link to="/" className="logohead">
              <h2 className="logo_heading">Kids Land</h2>
            </Link>
          </div>
                ...
                {Toy.map((item) => {
                  ...
                  if(item.isWished === true){
                    ...
                    <div className="wishDiv1">
                        <div><img style={{width: "100px", height: '100px'}} src={item.img} alt=""/></div>
                        <div style={{marginLeft: '10px'}} ><p><strong>{item.title}</strong></p>
                          <p><strong>Price:-</strong> &#8377;<span>{item.price -((item.price * item.discount)/100)}</span>&nbsp;<s style={{ color: "#1f1d1d", fontSize: '15px' }}>{item.price}</s></p></div>
                    </div>

                    <div className="rmbtns"><button onClick={()=>{remove(item.id)}} className="Rfwb">Remove from Wishlist</button>&nbsp;<button onClick={()=>{move(item.id)}} className="mtcb">Move to Bag</button></div>
                    <hr/>
                 </>;
                  }else{
                    temp++
        ...
        <span className="themetogglebtn" style={{ cursor: "pointer" }} onClick={changeTheme}>
              {theme ? <DarkModeIcon sx={{fontSize: '30px'}} /> : <LightModeIcon sx={{fontSize: '30px'}}/>}
          </span>
      </div>
    </>
  );
